...
describe('FormFieldsService', () => {
  ...
  let form: FormGroup = formbuilder.group({
    ...
    message: ['', [required, maxLength(1000), minLength(12)]],
    number: ['', [min(2)]]
  })
  ...
  it('should return min length', () => {
    form.controls['message'].setValue('Hey');
    let message = service.processFieldValidationMessage(form, 'message', content);
    expect(message).toEqual('minLength 12');
  });

  it('should return min length', () => {
    form.controls['name'].setValue('H');
    let message = service.processFieldValidationMessage(form, 'name', content);
    expect(message).toEqual('minLength 2');
  });
  ...
  it('should not return an error message', () => {
    form.controls['name'].setValue('Hi!');
    let message = service.processFieldValidationMessage(form, 'name', content);
    expect(message).toEqual('');
  });
